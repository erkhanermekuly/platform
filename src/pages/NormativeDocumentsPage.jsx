  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      resourcesAPI.documents
        ...
        .catch((e) => {
          if (!cancelled) setError(e?.message || 'Не удалось загрузить документы');
        })
        .finally(() => {
  ...
  return (
    <div className="resource-page">
      <header className="resource-page-header">
        ...
        <p>
          Официальные положения, регламенты и методические указания платформы. Список ведётся в базе данных;
          администратор может добавлять и обновлять записи.
        </p>
        {isAdmin ? (
          <p style={{ marginTop: 12 }}>
            <Link to="/admin/normative-documents" className="btn btn-primary">
              Управление документами (админ)
            </Link>
          </p>
        ) : null}
      </header>
      ...
      <div className="resource-toolbar">
        ...
        <select className="resource-filter" value={filter} onChange={(e) => setFilter(e.target.value)}>
          ...
          <option value="with_link">Только со ссылкой или файлом</option>
          <option value="without_link">Без ссылки и файла</option>
        </select>
      ...
      {!loading && !error && sortedItems.length === 0 && (
        <p className="resource-empty">
          Пока нет документов. Администратор добавит их в разделе «Управление документами».
        </p>
      )}
          ...
          <article key={item.id} className="resource-list-card">
            ...
            {item.attachedFileName ? (
              <button
                ...
                onClick={() =>
                  downloadResourceAttachedFile('documents', item.id, item.attachedFileName).catch((e) =>
