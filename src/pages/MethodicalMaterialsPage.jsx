...
export default function MethodicalMaterialsPage() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      setLoading(true);
      setError(null);
      resourcesAPI.materials
        .list()
        .then((res) => {
          if (!cancelled) setItems(Array.isArray(res?.data) ? res.data : []);
        })
        .catch((e) => {
          if (!cancelled) setError(e?.message || 'Не удалось загрузить материалы');
        })
        .finally(() => {
  ...
  return (
    <div className="resource-page">
      <header className="resource-page-header">
        ...
        <p>Консультационные материалы, шаблоны и полезные ресурсы для ежедневной практики.</p>
      </header>
      ...
      {!loading && !error && sortedItems.length === 0 && (
        <p className="resource-empty">Пока нет материалов. Администратор может добавить их через админ-панель.</p>
      )}
          ...
          <article key={item.id} className="resource-list-card">
            ...
            {item.attachedFileName ? (
              <button
                ...
                onClick={() =>
                  downloadResourceAttachedFile('materials', item.id, item.attachedFileName).catch((e) =>
